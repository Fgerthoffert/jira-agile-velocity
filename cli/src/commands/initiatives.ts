...
export default class Roadmap extends Command {
	...
	static flags = {
		...
		cache: flags.boolean({
			char: 'c',
			description: 'Use cached version of the child issues (mostly useful for dev)',
			default: false
		})
	};
	...
	async run() {
		...
		if (cache) {
			this.log('==================================================================================');
			this.log('Will be fetching data from cache. NO CALLS WILL BE MADE TO JIRA TO REFRESH DATA ');
			this.log('==================================================================================');
		}
		...
		for (const team of userConfig.teams.filter(
			...
			const teamIssues = await fetchCompleted(userConfig, this.config.configDir + '/cache/', team.name);
		...
		const initiativesIssues = await fetchInitiatives(userConfig, cacheDir, cache);
		...
		for (const initiative of initiativesIssues) {
			...
			const children = await fetchChildren(userConfig, initiative.key, cacheDir, cache);
		...
		// Export data to file
		const initiativesRawFileStream = fs.createWriteStream(path.join(cacheDir, 'initiatives-artifacts-raw.json'), {
			flags: 'w'
		});
		initiativesRawFileStream.write(JSON.stringify(rawInitiativesArtifact));
		initiativesRawFileStream.end();

		const initiativesSlimFileStream = fs.createWriteStream(path.join(cacheDir, 'initiatives-artifacts.json'), {
			flags: 'w'
		});
		...
		const initiativesSlimFileStreamArchive = fs.createWriteStream(
			path.join(cacheDir, 'initiatives-artifacts-' + new Date().toJSON().slice(0, 10) + '.json'),
			{ flags: 'w' }
		);
