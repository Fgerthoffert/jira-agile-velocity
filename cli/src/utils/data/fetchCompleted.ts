...
const returnTicketsPoints = (issue: any, config: IConfig) => {
	if (issue.fields[config.jira.fields.points] !== undefined && issue.fields[config.jira.fields.points] !== null) {
...
*/
const fetchCompleted = async (config: IConfig, cacheDir: string, teamName: string, toDate?: string | undefined) => {
	...
	if (teamConfig !== undefined) {
		...
		for (const scanDay of dates) {
			if (excludeDays.find((day: string) => day === scanDay.date.toJSON().slice(0, 10)) === undefined) {
				const issuesDayFilepath = path.join(
					cacheDir,
					'completed-' + getTeamId(teamName) + '-' + scanDay.date.toJSON().slice(0, 10) + '.ndjson'
				);
				...
				if (!fs.existsSync(issuesDayFilepath)) {
					...
					const jqlQuery = teamConfig.jqlCompletion + ' ON(' + scanDay.date.toJSON().slice(0, 10) + ')';
