...
class Config(object):
    ...
    def __init__(self, log):
        ...
        if os.path.isdir(self.configPath) == False or os.path.isfile(self.configFilepath) == False:
            self.initConfig()
        elif os.path.isfile(self.configFilepath) == True:
        ...
        self.schema = {
            ...
            , 'properties': {
                'history_weeks': {'type': ['number'], 'description': 'Number of weeks of history to process from current date', 'default': 12}
                , 'cache_filepath': {'type': ['string'], 'description': 'Path of the cache file', 'default': self.configPath + 'data.jsonl'}
                , 'jira_username': {'type': ['string', 'null'], 'description': 'JIRA Username used for the API call'}
                , 'jira_password': {'type': ['string', 'null'], 'description': 'JIRA Password used for the API call'}
                , 'jira_host': {'type': ['string', 'null'], 'description': 'Remote JIRA Hostname (https://jira.yourdomain.tld)'}
                , 'jira_jql_velocity': {'type': ['string', 'null'], 'description': 'JIRA JQL Query used to track story points completion (for example: project = WORK and status changed to Done)'}
                , 'jira_jql_remaining': {'type': ['string', 'null'], 'description': 'JIRA JQL Query used to determine remaining work (for example: sprint in openSprints() and type in (Story, Task, Defect))'}
                , 'jira_field_points': {'type': ['string', 'null'], 'description': 'Field used by JIRA to store Story Points', 'default': 'customfield_10002'}
                , 'slack_channel': {'type': ['string', 'null'], 'description': 'Slack channel to post this message to', 'default': '#general'}
                , 'slack_webhook': {'type': ['string', 'null'], 'description': 'Slack webhook (https://api.slack.com/incoming-webhooks)'}
            }
        }
