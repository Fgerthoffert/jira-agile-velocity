...
export default class Fetch extends Command {
  ...
  async run() {
    ...
    dataObject = await this.populateObject(dataObject, jira_points, jira_jql_completion); // Add completion counts to the object
    dataObject = await this.populateOpen(dataObject, jira_points, jira_jql_remaining); // Add open issues counts to the object
    ...
    this.log('====================== IN POINTS ===========================')
    this.log('Remaining Points: ' + dataObject.open.points + ' (' +  dataObject.open.issues.length + ' tkts)');
    this.log('On ' + dataObject.slack.previousDay.weekday + ', we completed: ' + dataObject.slack.previousDay.completed.points.count + ' pts (' + dataObject.slack.previousDay.completed.issues.count + ' tickets) [Max: ' + dataObject.slack.previousDay.completed.points.max + ' pts / Min: ' + dataObject.slack.previousDay.completed.points.min + ' / Avg: ' + dataObject.slack.previousDay.completed.points.avg + ' for ' + dataObject.slack.previousDay.weekday + 's]');
    this.log('Our daily velocity is at ' + dataObject.slack.previousDay.velocity.points.count + ' pts/day and is going: ' + dataObject.slack.previousDay.velocity.points.trend);
    this.log('This week (' + dataObject.slack.previousWeek.weekYear + ') we completed: ' + dataObject.slack.previousWeek.completed.points.count + ' pts [Max: ' + dataObject.slack.previousWeek.completed.points.max + ' / Min: ' + dataObject.slack.previousWeek.completed.points.min + ' / Avg: ' + dataObject.slack.previousWeek.completed.points.avg + ']');
    this.log('Our weekly velocity is at ' + dataObject.slack.previousWeek.velocity.points.count + ' pts/week and is going: ' + dataObject.slack.previousWeek.velocity.points.trend);
    this.log('Estimated sprint completion in: ' + dataObject.slack.timeToCompletion.completion.points.effort + ' days');    
    this.log('=================== IN TICKET COUNT =======================')
    this.log('Remaining Tickets: ' + dataObject.open.issues.length + ' (' +  dataObject.open.points + ' pts)');
    this.log('On ' + dataObject.slack.previousDay.weekday + ', we completed: ' + dataObject.slack.previousDay.completed.issues.count + ' tkts [Max: ' + dataObject.slack.previousDay.completed.issues.max + ' tkts / Min: ' + dataObject.slack.previousDay.completed.issues.min + ' / Avg: ' + dataObject.slack.previousDay.completed.issues.avg + ' for ' + dataObject.slack.previousDay.weekday + 's]');
    this.log('Our daily velocity is at ' + dataObject.slack.previousDay.velocity.issues.count + ' tkts/day and is going: ' + dataObject.slack.previousDay.velocity.issues.trend);
    this.log('This week (' + dataObject.slack.previousWeek.weekYear + ') we completed: ' + dataObject.slack.previousWeek.completed.issues.count + ' tkts [Max: ' + dataObject.slack.previousWeek.completed.issues.max + ' / Min: ' + dataObject.slack.previousWeek.completed.issues.min + ' / Avg: ' + dataObject.slack.previousWeek.completed.issues.avg + ']');
    this.log('Our weekly velocity is at ' + dataObject.slack.previousWeek.velocity.issues.count + ' pts/week and is going: ' + dataObject.slack.previousWeek.velocity.issues.trend);
    this.log('Estimated sprint completion in: ' + dataObject.slack.timeToCompletion.completion.issues.effort + ' days');    

    const cacheDir = this.config.configDir + "/cache/";
    const issueFileStream = createWriteStream(path.join(cacheDir, "artifact.json"),{ flags: "w" });
  ...
  populateOpen = async (dataObject, jira_points, jira_jql_remaining) => {
    ...
    let remainingPoints = issuesJira.filter(issue => issue.fields[jira_points] !== undefined).map(issue => issue.fields[jira_points]).reduce((acc, points) => acc + points, 0);
  ...
  slackMetrics = (dataObject) => {
    ...
    for (let i = dataObject.days.length - 1; i >=0 ; i--) {
      ...
      if (day !== 0 && day !== 6 && lastDay === false) {
        ...
        if (dataObject.days[i].completion.points.velocity > dataObject.days[i-1].completion.points.velocity) {
          velocityTrendPoints = 'UP'
        } else if (dataObject.days[i].completion.points.velocity === dataObject.days[i-1].completion.points.velocity) {
          velocityTrendPoints = 'FLAT'
        }
        let velocityTrendIssues = 'DOWN'
        if (dataObject.days[i].completion.issues.velocity > dataObject.days[i-1].completion.issues.velocity) {
          velocityTrendIssues = 'UP'
        } else if (dataObject.days[i].completion.issues.velocity === dataObject.days[i-1].completion.issues.velocity) {
...
//        console.log(dataObject.days[i])
    ...
    lastDayCompleted.completed.points.avg = Math.round(referenceDaysPoints.reduce((a,b) => a + b, 0) / referenceDaysPoints.length * 100) / 100
    ...
    lastDayCompleted.completed.issues.avg = Math.round(referenceDaysCount.reduce((a,b) => a + b, 0) / referenceDaysCount.length * 100) / 100
    ...
    for (let i = dataObject.weeks.length - 1; i >=0 ; i--) {
      ...
      if (lastWeek === false) {
        //get Velocity trend
        let velocityTrendPoints = 'DOWN'
        if (dataObject.weeks[i].completion.points.velocity > dataObject.weeks[i-1].completion.points.velocity) {
          velocityTrendPoints = 'UP'
        } else if (dataObject.weeks[i].completion.points.velocity === dataObject.weeks[i-1].completion.points.velocity) {
          velocityTrendPoints = 'FLAT'
        }
        let velocityTrendIssues = 'DOWN'
        if (dataObject.weeks[i].completion.issues.velocity > dataObject.weeks[i-1].completion.issues.velocity) {
          velocityTrendIssues = 'UP'
        } else if (dataObject.weeks[i].completion.issues.velocity === dataObject.weeks[i-1].completion.issues.velocity) {
          velocityTrendIssues = 'FLAT'
        }        
        lastWeek = true;
        lastWeekCompleted = {
          weekYear: getYear(new Date(dataObject.weeks[i].date)) + '.' + getWeek(new Date(dataObject.weeks[i].date)), 
          completed: {
            points: {
    ...
    lastWeekCompleted.completed.points.avg = Math.round(referenceWeeksPoints.reduce((a,b) => a + b, 0) / referenceWeeksPoints.length * 100) / 100
    ...
    lastWeekCompleted.completed.issues.avg = Math.round(referenceWeeksCount.reduce((a,b) => a + b, 0) / referenceWeeksCount.length * 100) / 100

    dataObject['slack'] = {previousDay: lastDayCompleted, previousWeek: lastWeekCompleted, timeToCompletion: dataObject['velocity'].filter(v => v.range === dataObject['defaultVelocity'])[0]};
...
//    console.log(JSON.stringify(ticketsPerDay));
  ...
  initObject = (fromDay: Date, toDay: Date) => {
    let initObject = { days: {}, weeks: {}, open: {}, velocity: {}, slack: {} };
  ...
  populateObject = async (dataObject, jira_points, jira_jql_completion) => {
    ...
    for (let [dateKey, dateData] of Object.entries(dataObject.days)) {
      const issues = await this.getCompletedIssues(dateKey, jira_jql_completion)
      ...
      for (let issue of issues) {
        ...
        if (issue.fields[jira_points] !== undefined && issue.fields[jira_points] !== null) {
          dataObject["days"][dateKey]["completion"]["points"]["count"] +=
        ...
        if (
          ...
          dataObject["days"][dateKey]["scopeChangeCompletion"]["issues"]["count"]++;
          dataObject["days"][dateKey]["scopeChangeCompletion"]["list"].push(issue);
          if (issue.fields[jira_points] !== undefined && issue.fields[jira_points] !== null) {
            dataObject["days"][dateKey]["scopeChangeCompletion"]["points"]["count"] += parseInt(issue.fields[jira_points]);
          }
        }
        ...
        if (dataObject["weeks"][closedWeekKey] !== undefined) {
          ...
          if (issue.fields[jira_points] !== undefined && issue.fields[jira_points] !== null) {
            dataObject["weeks"][closedWeekKey]["completion"]["points"]["count"] +=
            parseInt(issue.fields[jira_points]);
          }
          ...
          if (
            ...
            dataObject["weeks"][closedWeekKey]["scopeChangeCompletion"]["issues"][
              "count"
            ]++;
  ...
  */
  getCompletedIssues = async (dateToFetch, jira_jql_completion) => {
    const issues = []
    const cacheDir = this.config.configDir + "/cache/";
    if (!fs.existsSync(path.join(cacheDir, "completed-" + dateToFetch + ".ndjson"))) {
      cli.action.start(
      ...
      const input = fs.createReadStream(path.join(cacheDir, "completed-" + dateToFetch + ".ndjson"));
  ...
  fetchDataFromJira = async (jqlQuery) => {
    const { args, flags } = this.parse(Fetch);
    const userConfig = await loadYamlFile(
      path.join(this.config.configDir, "config.yml")
    );
...
export const calculateAverageVelocity = (array, category, indexValue) => {
  ...
  console.log(array.map(values => values[category][indexValue]['count']));
  console.log(array
