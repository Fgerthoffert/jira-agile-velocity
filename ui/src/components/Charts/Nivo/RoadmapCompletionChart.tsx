...
class RoadmapCompletionChart extends Component<any, any> {
  ...
  buildDataset = () => {
    ...
    for (const initiative of roadmap.initiatives.filter(
      ...
      for (const week of initiative.weeks) {
        initiativeData[week.weekEnd] = week[metric].count;
      }
    ...
    for (const week of roadmap.orphanIssues) {
      // @ts-ignore
      nonInitiatives[week.weekEnd] = week[metric].count;
    }
  ...
  render() {
    ...
    const chartHeight =
      50 +
      roadmap.initiatives.filter((i: any) => i.metrics[metric].completed > 0)
        .length *
        20;
    ...
    return (
      <div style={{ height: chartHeight }}>
        <ResponsiveHeatMap
          data={this.dataset}
          keys={roadmap.initiatives[0].weeks.map((w: any) => w.weekEnd)}
          indexBy="initiative"
