...
class RoadmapFutureChart extends Component<any, any> {
  ...
  */
	getCompletionColor = (data: any, value: any) => {
		...
		const initiative = roadmap.futureCompletion.find((i: any) => getInitiativeTitle(i) === data.yKey);
	...
	render() {
		...
		const initiatives = roadmap.futureCompletion.filter((i: any) => i.metrics[metric].remaining > 0);
