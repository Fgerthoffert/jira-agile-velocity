...
export const assignees = createModel({
  state: {
    teams: [],
    selectedTeam: null,
    loading: false,
  ...
  effects: {
    async initView(currentTab, rootState) {
      ...
      if (rootState.assignees.teams.length > 0) {
        return true;
      }
    ...
    async loadVelocityFromCache(teamId) {
      // If previous data was loaded and saved in localstorage
      // it will first display the cache, while the call to the backend is happening
      const cacheVelocity = reactLocalStorage.getObject('cache-assignees');
      if (Object.keys(cacheVelocity).length > 0 && cacheVelocity.find((t: any) => t.id === teamId) !== undefined) {
        ...
        this.setVelocity(reactLocalStorage.getObject('cache-assignees'));
      }
    },

    async refreshTeams(currentTab, rootState) {
      if (rootState.assignees.loading === false) {
    ...
    async fetchTeamData(teamId, rootState) {
      ...
      if (rootState.assignees.loading === false) {
        ...
        const setLoading = this.setLoading;
        if (
          rootState.assignees.teams.find((t: any) => t.id === teamId) !==
          undefined
        ) {
          const log = rootState.global.log;
          ...
          axios({
            method: 'get',
            url: host + '/assignees/' + teamId,
            headers,
          })
            .then(response => {
              const updatedVelocity = [
                ...rootState.assignees.velocity.filter(
                  (t: any) => t.id !== teamId,
                ),
                response.data,
              ];
              setVelocity(updatedVelocity);
              reactLocalStorage.setObject('cache-assignees', updatedVelocity);
              setLoading(false);
            })
            .catch(error => {
              console.log(error);
