	...
	effects: {
		...
		async loadDataFromCache(roadmap, rootState) {
			...
			if (Object.keys(cacheRoadmap).length > 0) {
				log.info('Loading Roadmap data from cache while call to the backend is happening');
		...
		async loadData(roadmap, rootState) {
			...
			if (
				...
				(JSON.parse(window._env_.AUTH0_DISABLED) !== true && rootState.global.accessToken !== '')
			) {
				setLoading(true);
				const host = window._env_.API_URL !== undefined ? window._env_.API_URL : 'http://127.0.0.1:3001';
				const headers =
					JSON.parse(window._env_.AUTH0_DISABLED) !== true
				...
				log.info('Not loading data, either there is already some data in cache or user token not present');
			}
		},
				...
				initiatives: roadmap.initiatives.map((i: any) => {
					return {
						...
						weeks: roadmap.calendar.completed.map((w: any) => {
							const initiativeWeek = i.weeks.find((iw: any) => iw.weekStart === w.weekStart);
				...
				futureCompletion: roadmap.futureCompletion.map((i: any) => {
					return {
						...
						weeks: roadmap.calendar.roadmap.map((w: any) => {
							const roadmapWeek = i.weeks.find((iw: any) => iw.weekStart === w.weekStart);
